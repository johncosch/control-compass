...
interface CompanyCardProps {
  company: {
    ...
    locationsServed?: Array<{ country: string; state: string | null; region: string | null }>
  }
}
...
const countryLabels = {
  ...
  CA: "Anywhere in Canada",
  MX: "Anywhere in Mexico",
}

export function CompanyCard({ company }: CompanyCardProps) {
  // Get unique areas served for display
  const getAreasServed = () => {
    if (!company.locationsServed || company.locationsServed.length === 0) return null

    const areas = company.locationsServed.map((location) => {
      if (location.state) {
        return location.state
      }
      return countryLabels[location.country as keyof typeof countryLabels] || location.country
    })
  ...
  const areasServed = getAreasServed()
      ...
      <CardContent className="pt-0">
        ...
        {areasServed && areasServed.length > 0 && (
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-1">
              {areasServed.slice(0, 3).map((area) => (
